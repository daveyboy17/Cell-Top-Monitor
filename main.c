/***********************************************************
	...
	Changes:
			v2	12 Jul 2016, Dave:
				made system sick active when load is on so that load can be cycled much faster
...
static void			fv_set_I2C1(uint8_t u8_type);
...
#if (0 != PROPORTIONAL_LOAD)
static void			fv_set_PWM(uint8_t u8_duty);
#endif
...
// HW_TYPE_CELL_MONITOR variables
uint16_t			u16_cell_voltage;						// should be around 3.650v when charged
int16_t				s16_cell_temperature;
uint16_t			u16_cell_max_mv;
uint16_t			u16_load_min_mv;
uint16_t			u16_cell_min_mv;
uint16_t			u16_cell_max_temp;
...
int main(void)
{
	...
	fv_set_I2C1(u8_hardware_type);							// could use this to implement SMBus.
	...
	u8_sleep_not_stop			= 0;
	u8_load_state				= 0;
	// BattMan should adjust these limits
	u16_cell_max_mv				= CELL_MAX_MV;
	u16_load_min_mv				= LOAD_MIN_MV;
	u16_cell_min_mv				= CELL_MIN_MV;
	u16_cell_max_temp			= CELL_MAX_TEMP;
	u16_mean_cell_mv			= 0;
...
/***********************************************************
	returns the hardware type identifier.
	called by db_table.c
***********************************************************/
uint16_t		fu16_get_hw_type(void)
{
	return u8_hardware_type;
} // end of fu16_get_hw_type -------------------------------
...
#else
#if (0 != PROPORTIONAL_LOAD)
	GPIO_InitStruct.GPIO_Pin		= GPIO_Pin_0 | GPIO_Pin_6;		// TODO: move load to PA6 so we can PWM it using TIM3
#else
...
static void			fv_set_timers(uint8_t u8_type)
{
	...
	// any type specific configuration goes here.
	switch (u8_type)
	{
	default:
		u16_period			= 0;
...
/***********************************************************
	@brief	Sets the I2C to suit the hardware type.
	@param	the hardware type identifier.
	@retval	None.
***********************************************************/
static void			fv_set_I2C1(uint8_t u8_type)
{
	// TODO: we may use this if we fit an EEPROM to store parameters.
} // end of fv_set_I2C1 ------------------------------------
...
static void			fv_set_RTC(uint8_t u8_type)
{
	...
	if (u8_type == HW_TYPE_CELL_MONITOR)
	{
...
static void			fv_read_ADC(uint8_t u8_type)
{
	...
	case	HW_TYPE_CELL_MONITOR:
		...
		// set the load state
		if (u16_mean_cell_mv > CELL_MIN_MV)
		{
			// are we > 50mv above the mean
			if (u16a_adc_buf[CELL_VCELL_INDEX] > ((u16_mean_cell_mv + 50) * ADC_MAX) / CELL_VOLT_MAX_INPUT)
			{
				u8_load_state		= 1;
			}
			else if (u16a_adc_buf[CELL_VCELL_INDEX] < (u16_mean_cell_mv * ADC_MAX) / CELL_VOLT_MAX_INPUT)
			{
				u8_load_state		= 0;
			}
		}
...
#if (0 != PROPORTIONAL_LOAD)
			++u8_load_state;
			if (u8_load_state > LOAD_MAX)
			{
				u8_load_state		= LOAD_MAX;
			}
#else
		...
		if (u16a_adc_buf[CELL_VCELL_INDEX] < (LOAD_MIN_MV * ADC_MAX) / CELL_VOLT_MAX_INPUT)
		{
#if (0 != PROPORTIONAL_LOAD)
			if (u8_load_state > 0)
			{
		...
		} // end of else mean cell mv has NOT been set
		...
		// set the load output
#if (0 != PROPORTIONAL_LOAD)
		// make the load proportional by using a PWM output, TIM3 is available at PA6, PA7 and PB1
		fv_set_PWM(u8_load_state);
#else
		...
		// set the alarm state
		u8_alarm_state				= 0;
		if (u16a_adc_buf[CELL_VCELL_INDEX] < (CELL_MIN_MV * ADC_MAX) / CELL_VOLT_MAX_INPUT)
		{
	...
	if (u8_UART1_rx_wp != u8_UART1_rx_rp)
	{
		...
		if ((u8_num_bytes % 6) == 0)
		{
					...
					&& ((u8a_buf[0] & 0x60) == 0) )			// the cell voltage can never get high enough to set these bits; (>8v)
			{
				...
				u16_mean_cell_mv					= (((u8a_buf[0] & 0xE0) << 8) + u8a_buf[1]);
#if 0
				/*	u8a_buf[0] can address up to 31 cells
					u8a_buf[2] can address up to 1 cell
					u8a_buf[1] contains the cell number
					u8a_buf[3] contains the load percentage	 */

				// check if the master message is for us.
				if ( (CELL_NUM_NOT_SET != u8_cell_number)
						&& (u8a_buf[1] == u8_cell_number) )
				{
					// set the load to the value in u8a_buf[1]
					u8_load_state					= u8a_buf[3];		// TODO: this will probably get overridden at present.
				}
#endif
			}
...
static void			fv_set_PWM(uint8_t u8_duty)
{
	...
	// Time base configuration
	TIM_TimeBaseStructure.TIM_Period			= (LOAD_MAX + 1);
	...
	TIM_OCInitStructure.TIM_Pulse				= u8_duty;
...
void			USART1_IRQHandler(void)
{
	...
	if(USART_GetITStatus(USART1, USART_IT_TC) != RESET)
	{
	    ...
	    if (u8_comms_mode == COMMS_TRANSMITTING)
	    {
