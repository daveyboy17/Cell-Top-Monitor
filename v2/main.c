...
static void			fv_append_data(uint8_t u8_type);
    ...
    while(1)
    {
		if (0 != u8_sys_tick_f)								// takes ~700us for cell monitor @ 8MHz
		{
			...
			if (u16_comms_intrvl_tmr >= AUTOSEND_SECONDS)	// u16_comms_intrvl_tmr is reset by fv_append_data()
			{
				...
				// default to voltage info unless we're sending both anyway.
				if (u8_msg_type == TYPE_TEMP_ONLY)
				{
					u8_msg_type			= TYPE_VOLTAGE_ONLY;
				}
				fv_append_data(u8_msg_type);				// this will also zero u16_comms_intrvl_tmr
					...
					/*&& (u8_comms_mode != COMMS_IDLE)*/ )
			{
				...
				u16_tick_cntr			= 0;
				// 1 Sep 2017, Dave - need to take us out of sleep as well.
				u8_sleep_not_stop		= 0;

				// TODO: could do a software reset instead
			}
...
static void			fv_set_timers(uint8_t u8_type)
{
	...
	if (u16_period != 0)
	{
		...
		TIM_OCInitStructure.TIM_Pulse				= u16_pulse;		// we set this to UART_SLEEP_MS when we see data
...
static void			fv_process_comms(void)
{
	...
	if (u8_UART1_rx_wp != u8_UART1_rx_rp)
	{
		...
		if (u8_num_bytes >= 3)
		{
			...
			fu8_fec_decode(&u8a_USART1_rx_buf[u8_UART1_rx_rp], &u8a_buf[0], 3);

//			if ( ((u8a_buf[0] & ALRM_ALL_BITS) == ALRM_ALL_BITS)	// no cell can have all 3 alarm bits set at once
...
//			}
			...
			if ((u8a_buf[0] & 0xF0) == 0xF0)
			{
				// request for temperature data
				u8_msg_type							= TYPE_TEMP_ONLY;
			}
			if ( (u8_msg_type == TYPE_TEMP_ONLY)
					&& ((u8a_buf[0] & 0xF0) == 0xE0) )
			{
				u8_msg_type							= TYPE_VOLTAGE_ONLY;
				u16_mean_cell_mv					= (((u8a_buf[0] & 0x0F) << 8) + u8a_buf[1]);
			}
...
static void			fv_append_data(uint8_t u8_type)
{
	...
	uint8_t		u8_i;
	uint8_t		u8_offset					= 0;
	uint8_t		u8_to_send					= 0;

	u8_comms_mode							= COMMS_TRANSMITTING;

	if (u8_type != TYPE_TEMP_ONLY)
	{
		// add the cell data
	...
	if (u8_type != TYPE_VOLTAGE_ONLY)
	{
		...
		u8a_buf[u8_offset]					= (s16_cell_temperature >> 8);
		u8a_buf[u8_offset + 1]				= (uint8_t) s16_cell_temperature;
		u8a_buf[u8_offset]					|= u8_alarm_state;

		u8_to_send							+= 3;
	}

	// add the fec bytes
	fu8_fec_encode(&u8a_buf[0], u8_offset + 2);

	// copy to the tx buffer
	for (u8_i = 0; u8_i < u8_to_send; ++u8_i)
	{
